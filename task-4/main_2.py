    ...
    def Thresholding(self):
        ...
        if self.comboBox.currentText()== "Local Otsu":
            ...
            if self.comboBox_H.currentText() != "None" and self.comboBox_W.currentText() != "None":
                localOptThreshold = self.localThresholding(npImg, 10, 10, self.otsuThresholding)
                Img = npImg > localOptThreshold

        elif self.comboBox.currentText()== "Global Otsu":
            Img = self.otsuThresholding(npImg)

        elif self.comboBox.currentText()== "Local Spectral":
            self.show_boxes()
            localOptThreshold = self.localThresholding(npImg, 10, 10, self.spectralThresholding)
            Img = npImg > localOptThreshold

        elif self.comboBox.currentText()== "Global Spectral":
            Img = self.spectralThresholding(npImg)

        elif self.comboBox.currentText()== "Local Optimal":
            self.show_boxes()
            localOptThreshold = self.localThresholding(npImg, 10, 10, self.getOptimmalThreshold)
            Img = npImg > localOptThreshold
            
        elif self.comboBox.currentText()== "Global Optimal":
            Img = self.globaloptimalThreshold(npImg)
            plt.imsave("./output/Global_Optimal.png",Img ,cmap = plt.cm.gray)

        else:
            return None
        
        # plt.imsave("./output/Global_Spectral.png",Img ,cmap = plt.cm.gray)
        # qpixmap = QPixmap("./output/Global_Spectral.png")
        # t2 = time.time()          
